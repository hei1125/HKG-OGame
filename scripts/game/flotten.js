...
function GetDistance() {
	...
	if (targetGalaxy - thisGalaxy != 0) {
		distance += Math.abs(targetGalaxy - thisGalaxy) * 25000;
	}
	if (targetSystem - thisSystem != 0) {
		distance += Math.abs(targetSystem - thisSystem) * 4 * 10 + 2500;
	} 
	if (targetPlanet - thisPlanet != 0) {
		distance += Math.abs(targetPlanet - thisPlanet) * 4 + 1000;
	} 
	return distance;
}
...
function GetConsumption() {
	...
	$.each(data.ships, function(shipid, ship){
		...
		basicConsumption = ship.consumption * ship.amount * data.fleetfuel;
