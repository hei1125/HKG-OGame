...
class statbuilder
{
	...
	private function GetTechnoPoints($USER) 
	{
		...
		foreach($reslist['tech'] as $Techno) 
		{
			...
			$Units	= $pricelist[$Techno]['cost'][901] * 0.25 + $pricelist[$Techno]['cost'][902] * 0.5 + $pricelist[$Techno]['cost'][903];
	...
	private function GetBuildPoints($PLANET) 
	{
		...
		foreach($reslist['build'] as $Build)
		{
			...
			$Units			 = $pricelist[$Build]['cost'][901] * 0.25 + $pricelist[$Build]['cost'][902] * 0.5 + $pricelist[$Build]['cost'][903];
	...
	private function GetDefensePoints($USER) 
	{
		...
		foreach(array_merge($reslist['defense'], $reslist['missile']) as $Defense) {
			...
			$Units			= $pricelist[$Defense]['cost'][901] * 0.25 + $pricelist[$Defense]['cost'][902] * 0.5 + $pricelist[$Defense]['cost'][903];
	...
	private function GetFleetPoints($USER) 
	{
		...
		foreach($reslist['fleet'] as $Fleet) {	
			...
			$Units			= $pricelist[$Fleet]['cost'][901] * 0.25 + $pricelist[$Fleet]['cost'][902] * 0.5 + $pricelist[$Fleet]['cost'][903];
