...
class ShowOverviewPage extends AbstractPage 
{
	...
	function show()
	{
		...
		if ($USER['onlinetime']  - 86400 > $USER['dmtime']){
			$GLOBALS['DATABASE']->query("UPDATE ".USERS." SET `dmtime` = ".$USER['onlinetime']." WHERE `id` = ".$USER['id'].";");
			$USER['darkmatter'] += 500;
		...
		$this->tplObj->assign_vars(array(
			...
			'servertime'				=> _date("Y-m-d H:i:s", TIMESTAMP, $USER['timezone']),
			'path'						=> HTTP_PATH,
		));
