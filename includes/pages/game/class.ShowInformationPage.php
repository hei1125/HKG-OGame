...
class ShowInformationPage extends AbstractPage
{
	...
	public function show()
	{
		...
		{
			if ($pricelist[$elementID]['tech'] == 1){
				$speedmultiplier = $USER['combustion_tech'] * 0.1;
			} elseif ($pricelist[$elementID]['tech'] == 2){
				$speedmultiplier = $USER['impulse_motor_tech'] * 0.2;
			} elseif ($pricelist[$elementID]['tech'] == 3){
				$speedmultiplier = $USER['hyperspace_motor_tech'] * 0.3;
			} else {
				$speedmultiplier = 0;
			}
			$FleetInfo	= array(
				...
				'curattack'		=> (int)($CombatCaps[$elementID]['attack'] * (1 + $USER['military_tech'] * 0.1)),
				'curshield'		=> (int)($CombatCaps[$elementID]['shield'] * (1 + $USER['shield_tech'] * 0.1)),
				'curstructure'	=> (int)(($pricelist[$elementID]['cost'][901] + $pricelist[$elementID]['cost'][902]) * (1 + $USER['defence_tech'] * 0.1)),
				'capacity'		=> $pricelist[$elementID]['capacity'],
				'speed1'		=> $pricelist[$elementID]['speed'],
				'speed2'		=> $pricelist[$elementID]['speed2'],
				'curspeed'		=> (int)($pricelist[$elementID]['speed'] * (1 + $speedmultiplier)),
