<div id="leftmenu">
	<ul id="menu">
		...
		<li><a href="game.php?page=achievements">{$LNG.lm_achievements}</a></li>
		<li><a href="game.php?page=moonbuy">{$LNG.lm_moonbuy}</a></li>
		<li><a href="game.php?page=findDebris">{$LNG.show_debris}</a></li>
		{if isModulAvalible($smarty.const.MODULE_SUPPORT)}<li><a href="game.php?page=market"style="color:yellow"><em>{$LNG['an_anres']}</em><em><font size="1" color="lime">   {$LNG['an_lotsum']} {$ddd}</font></em></a></li>{/if}
